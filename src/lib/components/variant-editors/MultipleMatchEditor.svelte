...
<div class="flex flex-col gap-4">
	...
	<div class="flex flex-col gap-2">
		...
		{/if}
		{#if !canAddMore}
			<p class="text-xs text-amber-600">Maximum {MAX_MULTIPLE_MATCH_ITEMS} items reached</p>
		{/if}
	</div>
			...
			<div class="flex flex-col gap-3">
				{#each items as item, index (item.id)}
					<div class="flex items-center gap-4">
						...
						<div class="flex h-[72px] flex-1 items-center rounded-lg border bg-white p-3 shadow-sm">
						...
						<div class="h-[72px] w-[200px] shrink-0">
							<input
								...
								class="h-full w-full rounded-lg border border-neutral-200 px-3 text-sm"
						...
						<button
							...
							class="flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-gray-400 transition-colors hover:bg-red-50 hover:text-red-600"
